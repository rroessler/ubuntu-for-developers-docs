...
workshop exec openjdk-example -- java out/App
```
...
cd helloworld && mvn -Dmaven.compiler.release=8 package
```
...
Press ENTER when projected for the Java version. This creates a new project which includes a `”Hello, World!”` application and a unit test that is replicated between the workshop environment and your host directory.
...
Since we have exposed the application through the configured slot/plug pairing, we can then see the application in any browser on the host machine at the [http://localhost:8080](http://localhost:8080) endpoint.
...
6. Refresh the workshop to now use the `maven` SDK instead of the `gradle` SDK:
